...
class ChatControllerTest {
    ...
    @MockkBean
    private lateinit var vtoroyService: VtoroyService
    ...
    fun `POST chat should return successful response`() {
        ...
        coEvery { 
            vtoroyService.chat(request.query, request.sessionId) 
        ...
        coVerify(exactly = 1) { vtoroyService.chat(request.query, request.sessionId) }
    }
    ...
    fun `POST chat should return bad request for invalid input`() {
        ...
        coVerify(exactly = 0) { vtoroyService.chat(any(), any()) }
    }
    ...
    fun `POST chat should handle large messages`() {
        ...
        coEvery { 
            vtoroyService.chat(largeQuery, request.sessionId) 
        } returns expectedResponse
    ...
    fun `POST chat should handle different sessions independently`() {
        ...
        coEvery { vtoroyService.chat("Hello", "session-1") } returns response1
        coEvery { vtoroyService.chat("Hi", "session-2") } returns response2
        ...
        coVerify(exactly = 1) { vtoroyService.chat("Hello", "session-1") }
        coVerify(exactly = 1) { vtoroyService.chat("Hi", "session-2") }
    }
}