...
class MainAgent(
    ...
    private val specializedAgents: List<Agent> = emptyList()
) : Agent {
    ...
    companion object {
        ...
        3. "delegate" - делегировать специализированному агенту (если запрос требует специфических действий)
        
        Правила принятия решения:
        - Приветствие, общие вопросы, ответ есть в истории → "dialogue"
        - Нужна информация из сохраненных документов/заметок → "knowledge_search"
        - Требуются специфические действия (создание заметок, работа с внешними системами) → "delegate"
        
        Примеры:
        - "привет, как дела?" → "dialogue"
        - "что такое Python?" → "dialogue"
        - "расскажи о проекте Jarvis из моих заметок" → "knowledge_search"
        - "найди информацию о встрече в моих документах" → "knowledge_search"
        - "создай новую заметку в Obsidian" → "delegate"
        - "отправь письмо" → "delegate"
    ...
    private fun handleDialogue(query: String, chatHistory: List<ChatMessage>): AgentResponse {
        ...
        4. Если нужна информация из базы знаний, предложи переформулировать запрос
        """
